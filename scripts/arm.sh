...
git clone https://github.com/malkist01/linaro-arm-eabi-4.9.git  -b master --depth=1 gcc
echo "Done"
...
# Compile plox
function compile() {
    make -s -C $(pwd) -j$JOBS KCFLAGS=-mno-android O=out teletubies_defconfig
    make -C $(pwd) CROSS_COMPILE_ARM32="${GCC}" KCFLAGS=-mno-android O=out -j$JOBS -l$LOADS 2>&1| tee build.log
        if ! [ -a "$IMAGE" ]; then
            finerr
            exit 1
        fi
